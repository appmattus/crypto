...
internal class SHA224 : Digest<SHA224> {
    ...
    private var hashObject: CC_SHA256_CTX = nativeHeap.alloc<CC_SHA256_CTX>().also {
        CC_SHA224_Init(it.ptr)
    }
    ...
    override fun update(input: ByteArray, offset: Int, length: Int) {
        if (length > 0) {
            input.usePinned {
                CC_SHA224_Update(hashObject.ptr, it.addressOf(offset), length.toUInt())
            }
        }
    }
    ...
    override fun digest(): ByteArray {
        ...
        digest.usePinned {
            CC_SHA224_Final(it.addressOf(0), hashObject.ptr)
        }
