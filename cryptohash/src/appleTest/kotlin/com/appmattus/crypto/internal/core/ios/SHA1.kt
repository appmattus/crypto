...
internal class SHA1 : Digest<SHA1> {
    ...
    private var hashObject: CC_SHA1_CTX = nativeHeap.alloc<CC_SHA1_CTX>().also {
        CC_SHA1_Init(it.ptr)
    }
    ...
    override fun update(input: ByteArray, offset: Int, length: Int) {
        if (length > 0) {
            input.usePinned {
                CC_SHA1_Update(hashObject.ptr, it.addressOf(offset), length.toUInt())
            }
        }
    }
    ...
    override fun digest(): ByteArray {
        ...
        digest.usePinned {
            CC_SHA1_Final(it.addressOf(0), hashObject.ptr)
        }
