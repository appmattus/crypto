...
internal class SHA384 : Digest<SHA384> {
    ...
    override fun update(input: ByteArray, offset: Int, length: Int) {
        if (length > 0) {
            input.usePinned {
                CC_SHA384_Update(hashObject.ptr, it.addressOf(offset), length.convert())
            }
        }
    }

    override fun digest(): ByteArray {
        val digest = UByteArray(CC_SHA384_DIGEST_LENGTH.convert())

        digest.usePinned {
            CC_SHA384_Final(it.addressOf(0), hashObject.ptr)
        }
